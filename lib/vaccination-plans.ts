...
// 0020_unified_visits.sql). Deliberately doesn't filter by tenant_id in
// the query — RLS already returns "global OR mine", which is exactly the
// built-in-plus-custom list this page shows.
import "server-only";
...
export async function getVaccinationPlans(
  ...
  const { data } = await supabase
    ...
    .select("id, tenant_id, species_group, vaccine_name, age_weeks_due, booster_interval_months, notes")
    .order("species_group")
    .order("vaccine_name");
